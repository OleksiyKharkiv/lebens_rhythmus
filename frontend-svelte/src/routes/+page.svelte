		...
		<div class="mt-10 grid gap-6 sm:grid-cols-3">
			{#each directions as d (d.title)}
				<div class="rounded-2xl border border-ink-line bg-ink px-6 py-8 transition-colors hover:border-gold/60">
					<!-- Direction cards typography calibrated in layout.css (LR-105):
					     Mobile (<640px) uses 1.25rem title / 1.125rem desc to prevent clumsy wrapping on 360-390px,
					     preserving the requested 1.5rem / 1.375rem base for >=640px up to 3840px. -->
					<h3 class="direction-card-title font-display font-semibold text-teal">
						{d.title()}
					</h3>
					<p class="direction-card-desc mt-3 leading-relaxed text-paper-dim">
						{d.desc()}
					</p>
				</div>
			{/each}
