...
## LR-084 — Backend+Frontend: `Enrollment`/`EnrollmentService`/`EnrollmentController`/`Order` — единая регистрация на `Workshop`+`Course`

**Tier:** HIGH (пересекает `payment`-смежную область — `Order`; влияет
на уже используемый в проде путь регистрации) · **Статус:** Closed
2026-08-16, коммит `f706ae8` · закрытие подтверждено `architect-reviewer`
финальным независимым прогоном 2026-08-16 (свежая проверка кода/тестов
с нуля, без опоры на предыдущие раунды)
**Источник:** `LR-068`/`LR-ADR-023` п.5 — реализовано через
founders'-roundtable по архитектуре регистрации (2026-08-16, полный
состав персон см. `decisions.md`), два раунда `architect-reviewer` на
этапе дизайна + по одному раунду на диффы бэкенда и фронтенда.

**Примечание для читающего архив:** запись в основном tickets.md к
моменту закрытия ещё содержала устаревшую пометку "Всё ещё открыт...
`Enrollment.workshop` по-прежнему `optional = false`, `course`-поля
нет" (артефакт-аудита от 2026-08-13, до фактической реализации) — это
исторический след, не актуальное состояние; см. ниже, что реально
сделано.

**Реализовано (весь scope из исходного тикета закрыт):**
1. `Enrollment.workshop` → nullable, новое nullable `course`-поле
   (`@ManyToOne` на `Course`) + новое nullable `order`-поле (`@ManyToOne`
   на `Order`) — решает и исходный п.1, и "недостроенную ветку"
   Order/Payment, которую заказчик явно потребовал закрыть в рамках
   этого же тикета (связано с дашбордом пользователя).
2. Ценовая развилка решена явно, не тихим побочным эффектом: цена
   берётся из `Course.price` (поле уже существовало из более раннего
   `LR-069`/`LR-081`). `price == null` → `EnrollmentStatus.CONFIRMED`
   немедленно, без создания `Order`; `price > 0` → `PENDING` +
   системный `Order` (курс/сумма/валюта), созданный в той же
   транзакции.
3. `EnrollmentController` — новый `POST /courses/{courseId}/enroll`,
   зеркалит существующий workshop-роут.
4. `Order.java` — получил `course`-поле; `orderNumber`-генерация (и
   здесь, и в `EnrollmentService.generateOrderNumber()`) усилена
   UUID-суффиксом против коллизии под конкурентной нагрузкой.
5. `uk_user_course` (уникальный констрейнт) добавлен параллельно
   `uk_user_workshop`, подкреплён app-level guard'ом
   (`existsByUserIdAndCourseId`) — п.5 исходного тикета закрыт.
6. П.6 исходного тикета (`countDistinctWorkshopsPerUserWithStatus`
   остаётся workshop-scoped) осознанно оставлен вне скоупа — так и было
   явно зафиксировано в самом тикете, не случайный пропуск.

**Сверх исходного текста тикета** (найдено на этапе roundtable-дизайна,
решено в этом же тикете по прямому решению заказчика):
- Атомарный захват вместимости группы —
  `GroupRepository.decrementCapacityLeft`/`incrementCapacityLeft`
  (`UPDATE ... WHERE capacity_left > 0`), закрывает реальный
  race condition (было check-then-save в двух отдельных операциях).
  `V12`-миграция бэкфиллит `capacity_left` из реальных существующих
  `Enrollment` — без этого шага атомарный гейт при деплое на прод с уже
  существующими записями молча позволил бы перебронирование.
- `GroupFullException`/`AlreadyEnrolledException` (409,
  `code: GROUP_FULL`/`ALREADY_ENROLLED`) заменили `raw
  RuntimeException`, который проваливался в generic `handleAll()` и
  всегда возвращал нечитаемую английскую строку 500-й ошибкой —
  особенно заметно для Course, где заполненная группа — обычный
  повседневный путь, а не редкий race для Workshop.
- `EnrollmentCleanupService` — первый `@Scheduled`-джоб в проекте,
  ежедневно (03:00) освобождает вместимость просроченных (>7 дней)
  неоплаченных `PENDING`-регистраций, переводит их в новый статус
  `EXPIRED`.
- **Security-фикс (реальная дыра, а не гипотетическая):**
  `OrderRequestDTO.status` копировался клиентским значением без
  проверки — любой аутентифицированный пользователь мог создать заказ
  сразу со `status: "PAID"`. `OrderService.create()` теперь
  безусловно форсирует `PENDING` после маппинга, игнорируя клиентский
  ввод; `update()` (уже был `ADMIN`/`BUSINESS_OWNER`-gated) не тронут.
  Зафиксировано как обязательное правило проекта — новый
  `CODING_PROTOCOL.md` §4b + F12-чеклист.
- **Frontend security-фикс:** `login/+page.svelte`'s `safeReturnTo()`
  был уязвим к open-redirect через WHATWG-URL backslash-normalization
  (`/\evil.com` → резолвится в `https://evil.com`) — строковая
  prefix-проверка заменена на `new URL(raw, location.origin)` +
  сравнение `.origin`, устойчивое независимо от того, каким методом
  (`goto()` или `window.location.href`) в итоге произойдёт переход.
- `EnrollButton.svelte` (новый общий компонент) — вынесена
  login-gate + API-dispatch + error-handling логика, использовалась бы
  иначе продублирована второй раз для Course.

**Follow-up тикеты, поданные отдельно (осознанно вне скоупа):**
`LR-087` (билеты на Performance — заблокировано отсутствием Gewerbe у
владелицы), `LR-088` (регистрация детей/согласие родителей),
`LR-089` (ревизия всех request-ДТО на предмет клиентски управляемых
статус/флаг-полей), `LR-090` (`GroupDTO.enrolledCount` разошёлся с
`capacityLeft` после этого тикета).

**Verify:** `./gradlew compileJava compileTestJava` + `./gradlew test
--tests "com.be.service.*"` — 67 тестов, 0 failures/errors (12 классов,
включая новые `EnrollmentServiceTest`, `EnrollmentCleanupServiceTest`,
`OrderServiceTest`). `npm run check` — 0 ошибок/предупреждений.
`npm test -- --run` — 13/13. Полный `./gradlew test` по-прежнему
показывает ~13 предсуществующих Docker/Testcontainers-провалов,
не связанных с этим тикетом (Docker недоступен в среде разработки).
Финальный независимый `architect-reviewer`-прогон (не опирался на
предыдущие раунды) подтвердил каждое заявленное в коммите изменение
построчно против реального кода — PASS по всем 6 проверенным
областям.

**Не сделано / известные небольшие хвосты (не блокировали закрытие):**
- Живой browser-E2E проход не подтверждён в этой сессии — preview-тул
  не резолвит рабочую директорию `frontend-svelte` (известная
  проблема окружения, см. `KNOWN_ISSUES.md`/сессионные заметки).
  Верификация — `svelte-check`+`vitest`, не живой рендер.
- Нет автотестов на `safeReturnTo()` и `EnrollButton` конкретно (только
  ручная верификация ревьювером через Node's URL parser) — регрессия
  сюда пройдёт незамеченной автотестами.
- `frontend-svelte/src/lib/api.ts`'s `EnrollmentAdminDTO` не объявлял
...
**Tier:** MED-HIGH · **Статус:** Closed 2026-09-07
**Источник:** прямой запрос заказчика п.4

Единственная пара `Start`/`Ende` в форме Workshop заменена на
"Количество дней" (0–10) → N под-форм (Start/Ende `datetime-local` +
`Venue`). Бэкенд/API-клиент (`SessionController`, `replaceSessions`)
уже существовали (найдено в коде — комментарий "New in LR-074") —
реализация переиспользует уже рабочий паттерн `admin/groups/
+page.svelte` (день-лист + venue на день) один-в-один, вместо
параллельной логики с нуля. Страница управляет одной "основной"
`Group` воркшопа (создаёт при первом сохранении) — тот же MVP-паттерн,
что уже у `admin/courses` (LR-081/082).

**Находка на этапе Architecture Pre-Check (не постфактум):**
`Workshop.startDate`/`endDate` по-прежнему используются
`WorkshopService.listWorkshops(upcoming=true)` для публичного списка —
Workshop не "schedule-free" как Course (LR-ADR-023). Решено и
подтверждено владельцем до кода: эти поля вычисляются автоматически
как min/max дней при сохранении, не вводятся вручную — иначе новые
воркшопы молча пропадали бы из публичного списка "предстоящих".

Teacher-дропдаун — уже был переключён на `getTeachers()` в LR-072 (не
в скоупе этого тикета, подтверждено чтением кода перед началом).

Убран ставший фактически неверным ключ `admin_workshop_venue_moved_note`
(все 3 языка) — venue снова задаётся прямо в форме воркшопа, по дням.

**Verify:** `npm run check` 1168 файлов/0 ошибок, `npm test` 13/13.
Backend не запущен в этой сессии — живая проверка недоступна, owner
тестирует сам в проде после деплоя.
