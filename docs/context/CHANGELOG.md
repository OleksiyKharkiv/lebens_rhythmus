...
Сразу после фикса редирект-петли (см. запись выше) сайт стартовал, но
приложение не монтировалось — консоль показывала CSP-violation на
инлайн-скрипты. LR-100's CSP считал, что в `index.html` есть ровно один
инлайн-скрипт (LR-019, анти-FOUC тема), и захардкодил под него 2
sha256-хэша (LF/CRLF варианты). На деле их минимум три:

- **бутстрап-скрипт самого `adapter-static`** (`__sveltekit_<random> = {...}`
  + `Promise.all([import(start.js), import(app.js)])`) — заблокирован,
  без него Svelte не монтируется вообще, отсюда "стартовал, но упал".
  Проверено локально: два чистых билда одного и того же коммита дали
  РАЗНЫЕ суффиксы переменной (`pg8m0r` и `7dkrge`, живьём в проде) —
  SvelteKit генерирует случайный токен на каждую сборку, не производный
  от контента файлов. Захардкоженный hash для этого скрипта
  принципиально не может быть актуальным дольше одного билда.
- **скрипт, вставляемый самим Cloudflare** на грани
  (`/cdn-cgi/challenge-platform/...`, похоже на Bot Fight Mode) — тело
  содержит случайный челлендж-токен на каждый ЗАПРОС, не билд. Хэш-пиннинг
  для него невозможен ни при каком построении CI.
- **fix (временный, разблокировать сайт)** — `script-src` заменён с
  `'self' 'sha256-...' 'sha256-...'` на `'self' 'unsafe-inline'` (по
  спеке CSP `'unsafe-inline'` игнорируется браузером при наличии
  hash-источника в той же директиве — нельзя было просто добавить его
  рядом со старыми хэшами, только заменить).
- **правильное решение (не сделано сейчас, отдельным тикетом)** —
  SvelteKit's native `kit.csp` (`mode: 'hash'`) считает хэши своих же
  инлайн-скриптов в момент сборки и может встроить корректный
  `<meta http-equiv="Content-Security-Policy">` прямо в билд — тогда
  hash никогда не будет стухшим вручную. Отдельно нужно решить вопрос
  Cloudflare-скрипта — либо выключить фичу, которая его вставляет
  (Bot Fight Mode/challenge platform в дашборде), либо смириться, что
  script-src для этого домена не может быть строже `'unsafe-inline'`,
  пока эта фича включена.
- **lesson** — оба сегодняшних прод-инцидента (редирект-петля +
  CSP-хэши) объединяет одно: инфра/security-правки из LR-100/LR-101
  ушли в прод без проверки в реальном браузере против реальной топологии
  (Cloudflare Tunnel, adapter-static build) — код-ревью и
  `architect-reviewer` не заменяют "открыть сайт и посмотреть, что в
  консоли", когда меняется CSP или Ingress-маршрутизация.
- **verify** — локальная сборка (`npm run build`) + сравнение с реальным
  прод-`index.html` из консоли инцидента, до пуша фикса.
...
- **docs** — круглый стол про идею "numi-API + QR" (раунды 3-4,
  2026-08-19) дописан в `LR-093` — `architect-reviewer` независимой
  проверкой нашёл, что этот раунд нигде не был записан и противоречил
  уже зафиксированной Фазе B (ручное подтверждение, N26 без публичного
  API) — исправлено этим коммитом, явно помечено как дополнение, не
  замена.
- **docs** — `LR-096` (Фаза A, Stripe Checkout) — все конкретные
  находки ревьювера (unique constraint на `event.id` для
  идемпотентности, проверка подписи по сырым байтам, `SecurityConfig`
  permitAll-вырез, состояние "есть незавершённая Stripe-сессия",
  единый юнит-тестируемый метод конвертации денег, GDPR/§4b-проверка
  Stripe-идентификаторов) — как обязательные пункты реализации, не
  общие пожелания.
- **docs** — `LR-097` (Фаза B, GiroCode-напоминание + вспомогательные
  фичи) — заблокирован на Steuerberater-подтверждении "напоминание vs
  Rechnung" (владелец подтвердил: ещё не спрошен). Алгоритм срока
  действия напоминания зафиксирован (min(3 дня, время до старта
  события)), плюс явный открытый вопрос про буфер на обработку
  перевода и про канало-осведомлённость `EnrollmentCleanupService`.

## 2026-08-19 — fix: тёмная тема — реальный файл вместо CSS-фильтра; шрифты на десктопе

### Область (`frontend-svelte/{static/background-dark.png (new),src/routes/{layout.css,+layout.svelte,+page.svelte}}`)

- **revert** — эксперимент с `invert()+hue-rotate()` (38f699c) откачен
  (`git revert`, не force-push, коммит `768c0fc`) — Олена подготовила
  отдельный файл под тёмную тему вместо мехинверсии.
- **feat** — `background-dark.png` подключён симметрично светлой теме,
  тот же `color-mix()`-wash, та же геометрия.
- **fix** — контраст меню (bg-ink/96%) и hero text-shadow (5e22b3e)
  оставлены как есть — оба были про "текст поверх пёстрого рисунка"
  в целом, не про конкретно откаченный фильтр, тот же риск актуален
  для нового файла.
- **fix** — шрифты на десктопе казались мелкими (фидбэк Олены).
  Проверил гипотезу "абсолютные vs относительные единицы" прежде чем
  что-то менять: уже `rem` везде (Tailwind v4 default type scale,
  `grep` подтвердил — ни одного `px`-размера, `@theme` шкалу не
  переопределяет) — контраргументировал: смена единиц измерения сама
  по себе размер не увеличивает, это про доступность/масштабирование
  под зум пользователя, не про "стало крупнее по умолчанию". Реальная
  причина — у пунктов меню вообще не было явного класса размера
  (наследовали 16px), а Hero был подогнан под `sm:` (640px), без шага
  для широких мониторов. Добавлен `text-lg` на десктопный `<nav>`
  (каскадом на все ссылки) и `lg:`-шаг для `h1`/подзаголовка Hero —
  мобильный/планшетный размер не тронут (проверено вживую на 375px).
- **verify** — `npm run check` 0 ошибок на каждом шаге. Живой браузер:
  тёмная тема грузит `background-dark.png` (200, не `-light`), размеры
  шрифта подтверждены на 1440px (18/60/20px) и 375px (36/18px,
  без изменений).
