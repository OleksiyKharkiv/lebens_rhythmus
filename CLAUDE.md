...
- **Docs** (всё под `docs/`, плюс этот файл): Russian по умолчанию. Исключение
  ...
  пишется по-английски независимо от языка разработчика. Та же логика и то же
  исключение — для **`AGENTS.md`** (добавлен 2026-09-14, докаудит) — эквивалент
  этого файла для Google Antigravity, не для человека-читателя.
